...
public final class FilesHelper {
   ...
   */
  public ImmutableMap<Path, StructuredTaskScope.Subtask<FileTime>> getLastModifiedTimeOfFiles(
      ImmutableList<Path> filePaths) throws InterruptedException {
    ImmutableMap.Builder<Path, StructuredTaskScope.Subtask<FileTime>> pathFileTimeFutures =
  ...
  /**
   * Opens {@link FileChannel}s for the provided file {@link Path}s with the provided {@link
   * StandardOpenOption}s.
   *
   * <p>The futures in the returned map are guaranteed to be done, i.e., {@link Future#isDone()}
   * will return {@code true}. Handling the state of the future ({@link Future#state()}) is the
   * responsibility of the caller.
   */
  public ImmutableMap<Path, StructuredTaskScope.Subtask<FileChannel>> openFileChannels(
      ImmutableList<Path> filePaths, ImmutableSet<StandardOpenOption> fileChannelOptions)
      throws InterruptedException {
    ImmutableMap.Builder<Path, StructuredTaskScope.Subtask<FileChannel>> pathFutureFileChannelMap =
        new ImmutableMap.Builder<>();
    try (var scope =
        new StructuredTaskScope.ShutdownOnFailure("open-file-channels", threadFactory)) {
      for (Path path : filePaths) {
        Callable<FileChannel> pathCallable = () -> FileChannel.open(path, fileChannelOptions);
